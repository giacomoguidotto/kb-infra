...
schedule of when to post) and the **intended tone for each slot**, and Social Compose reads them to recommend a slot and a slot-matched tone per draft — still
...
platform `<social-profile-source>` (currently X and LinkedIn) that KB Reconcile
...
update path and stalls. [ADR 0003](0003-social-compose-models-audience-state.md)
...
- **Have Social Compose schedule into the sink.** Rejected: it breaks the draft-only
  invariant ([ADR 0003](0003-social-compose-models-audience-state.md),
  ...
  free tier cannot schedule regardless. Social Compose *suggests* a slot; it does not commit one.
...
- **Read the profiles from Social Compose directly.** Rejected: KB Reconcile is
  ...
  ([ADR 0004](0004-kb-reconcile-many-sources-one-pattern.md)). A public source
  reconciling a public-facing surface belongs on that pattern, not bolted onto the
  draft loop.
- **Read the paid timeline APIs** — the mechanism
  [ADR 0003](0003-social-compose-models-audience-state.md) rejected. Still rejected:
...
- Social Compose reads the schedule and, for each candidate and draft, recommends a
...
- The preamble Sources vocabulary gains `<social-profile-source>` — read-only, per-
  platform, public. KB Reconcile declares it, observes it best-effort, and
...
- The flow is public-to-public — a public profile reconciling a public-facing ledger —
  so it does not touch KB Reconcile's private-by-default rule, which governs
  transcript-derived signals
  ([ADR 0004](0004-kb-reconcile-many-sources-one-pattern.md)).
- [ADR 0003](0003-social-compose-models-audience-state.md)'s rejection of platform
...
- Setup collects `<social-profile-source>` as a per-platform binding, like a sink; where
  a platform is unreadable the ledger update falls back to a KB Reconcile question rather than
  a fabricated entry.
