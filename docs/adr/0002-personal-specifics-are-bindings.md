...
and stored in gitignored `local/`, never committed. Two kinds of binding exist:
**sinks** (external systems an automation materializes into — a career system, a
portfolio repo, a social draft queue) and **KB endpoints** (the named context
surfaces and rule-sets an automation reads from the Knowledge Bank — selected
...
- `local/bindings.yml` (gitignored) is the single home for provider connection,
  sinks, endpoint bindings, cadences, and clone paths.
